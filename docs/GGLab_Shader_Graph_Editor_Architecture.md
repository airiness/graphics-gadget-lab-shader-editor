...
- The **WebView never constructs raw argv** and holds no generic `spawn`
  capability of any kind. The argument array is constructed structurally by the
  Toolchain Client from the toolchain's machine protocol.
- **Rust performs bounded execution only**: it runs a validated argument array
  (bounded count and length, never a shell string), captures the exit code and
  output bytes, and enforces timeout/cancel. It parses and judges no protocol
  content.
- **Protocol judgment exists exactly once, in the TypeScript Toolchain
  Client**: envelope parsing, status-vocabulary checking, version and
  compatibility judgment, and the readiness/build-state rules. The service's
  raw output is the client's only input.
- `launchPreview` remains a slot of the preview architecture (Slice 4+), not a
  Slice 2 surface.
...
the delivered bytes, compiles against it, and cleans it up. The WebView never
...
**Strict readiness gate (owner decision, 2026-08-24).** Readiness is an
explicit state ladder, and every rung is a visible state, never an implied one:

```text
unavailable   no candidate tool resolved by the discovery rules
...
unproven      resolved, but not machine-readably proven compatible; the honest
              ...
              exists and the client supports it
incompatible  resolved, and its facts contradict the required ones
ready         proven tool compatibility AND a compatible required descriptor
              AND host execution capability AND the explicit build target
              configured
```

The product compile path is gated on `ready` **with no bypass**: no dev mode,
environment flag, or local setting may route a compile request through an
unproven or incompatible tool; there is simply no such path to configure.

Composition (who decides — owner decision, 2026-08-24): the **editor
composition layer** composes readiness from its siblings — descriptor/profile
compatibility (judged by `shader-graph-core`), tool compatibility (judged by
the Toolchain Client), and host execution capability (reported by
`ShaderToolService`). There is exactly one composition point; no sibling
re-decides another's verdict.

Conceptually:

```text
Ready =
    compatible gglab-shaderc process contract
    AND compatible required Surface Profile Descriptor
    AND required deployment dependencies are resolvable
```
...
handshake contract is owned by the external Shader Toolchain authority
(`gglab-shaderc` / `ShaderToolchainCore`). **What the editor requires is not
what the editor owns.** This document records only the FACTS the editor's
compatibility verdict requires; it does not define, duplicate, or assign
values to the wire schema or to the toolchain's own version axes. Those facts:
...
unsupported or newer ones explicitly — the same reader discipline as the
descriptor reader: unknown or newer data fails explicitly, never silently
reinterpreted. Until it supports a published contract, its supported set is
empty, which is exactly why no real tool can be proven yet — and none may be
compiled through.
...
When a result returns, it only becomes **current** if it still matches the
current semantic/build revision — and the revision anchor is the
generated-source identity (the SHA-256 of the exact generated HLSL bytes, the
core's durable identity).
...
build configuration), stage / entry (descriptor facts), generated-source
identity (core emission), result states (session store over the client's rules),
artifact facts (tool result) — and the inspector only projects. A field with
...
1. Native build runs behind a **strict readiness gate** —
   `unavailable / discovered / unproven / incompatible / ready`. Until a tool
   is machine-readably proven compatible it may never enter the product
   compile path: no dev bypass, no environment-flag exception, and no such
   path to configure because none exists. A real tool before the toolchain
   handshake contract exists is `discovered-but-unproven`, full stop.
2. The handshake wire contract is **owned by the external Shader Toolchain
   authority**. This repository records only the editor's consumption
   requirements (the facts a compatibility verdict needs; the client's
   supported-range discipline) — no wire schema, no values assigned to the
   toolchain's own version axes. *What the editor requires is not what the
   editor owns.*
3. The Toolchain Client (`shader-toolchain-client`, a new headless package)
   ...
   direction. It owns machine-protocol consumption, tool-compatibility
   verdicts, build result/revision, and the stale/current/last-good pure
   rules — without understanding ShaderGraphDocument, GraphType, or node
   semantics. The editor composition layer is the single composition point:
   descriptor compatibility (core) + tool compatibility (client) + host
   readiness (service).
4. The desktop host exposes a **narrow `ShaderToolService`** (discover /
   ...
   never constructs raw argv; Rust performs bounded execution only; protocol
   judgment exists exactly once, in the TypeScript Toolchain Client.
5. Build-request composition: **target = explicit build configuration** (never
   derived from the descriptor; the first real target is `gglab-dx12`);
   **stage / entry = descriptor `generatedFunction` facts**; DXC and backend
   target policy remain `gglab-shaderc`'s. Generated HLSL is a derived build
   input staged in a private area owned by the service.
6. **`apps/cli` gains no `build` command in Slice 2.** Native-build
   orchestration for the CLI is a separately reviewed deferred addition. The
   fake-process test surface is the client / process-runner boundary
   directly.
...
- build revision/stale-result protection;
- artifact/manifest display;
- Build Inspector;
- fake-process tests at the Toolchain Client / process-runner boundary (the
  real tool is never a test fixture and never a test dependency);
- optional owner-run real-tool smoke, only after the toolchain handshake
  contract exists (first target: explicit `gglab-dx12`) — manual, recorded,
  never the CI default, never through an unproven tool.
...
→ gglab-shaderc
→ ShaderArtifact
→ artifact/build identity visible
```
