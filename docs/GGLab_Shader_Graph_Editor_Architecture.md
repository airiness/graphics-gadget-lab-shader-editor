...
- The **WebView never constructs raw argv** and holds no generic `spawn`
  ...
  (`NativeCompileRequest`: source bytes + identity, target, stage, entry);
...
artifact facts (tool result) — and the inspector only projects. In the Slice 2
generated-function qualification operation, stage / entry are descriptor facts;
they are not the executable Preview Program entry. A field with
...
**Program boundary clarification (2026-08-29).** The generated
`EvaluateSurface` function is a function contract, not a complete Runtime pixel
program. Compiling it as the descriptor entry remains valid Slice 2
generated-function qualification evidence, but the resulting artifact is not a
Runtime Preview Artifact. Slice 4 must compose the exact generated bytes with a
main-GGLab-owned Preview Program whose real pixel entry is `PSMain`; the Editor
must not synthesize that entry or pass binding. The proposed fixed-input
composition, publication, identity, and last-good contract is owned by the
GGLab docs repository in
`GGLab_Shader_Graph_Preview_Program_And_Lab_Design.md` and requires owner
...
4. The desktop host exposes a **narrow `ShaderToolService`** (discover /
   ...
   protocol interpretation exists exactly once, in the TypeScript Toolchain
   Client.
5. Slice 2 generated-function qualification request composition: **target =
   explicit build configuration** (never derived from the descriptor; the first
   real target is `gglab-dx12`); **stage / entry = descriptor
   `generatedFunction` facts**; DXC and backend target policy remain
   `gglab-shaderc`'s. Generated HLSL is a derived build input staged in a private
   area owned by the service. This operation does not produce the executable
   Slice 4 Preview Program. Preview uses a distinct main-owned Pixel/`PSMain`
   composition and publication contract.
...
- exploratory manual smoke — the owner may run `gglab-shaderc` outside the
  editor at any time, as investigative evidence only; it never changes the
  tool's `unproven` state and never enters the editor's product path;
- stage-acceptance smoke — only after the toolchain handshake contract
  exists: the real editor product path at `Ready` (first target: explicit
  `gglab-dx12`), manual, recorded, never the CI default.
...
→ generated HLSL
→ gglab-shaderc
→ generated-function qualification ShaderArtifact
